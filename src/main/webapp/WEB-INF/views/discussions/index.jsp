    ...
    <main class="discussion-column">
        ...
        <section class="chat-panel" aria-live="polite">
            <c:choose>
                ...
                <c:otherwise><div class="message-list"><div class="message-list-inner"><c:forEach var="chatMessage" items="${room.messages}"><c:set var="isOwn" value="${sessionScope.userId eq chatMessage.senderId}" /><article class="message-row ${isOwn ? 'outgoing' : 'incoming'}">
                    <c:choose>
                        <c:when test="${isOwn}">
                            <div class="message-bubble outgoing-bubble">
                                <header class="message-meta">
                                    <strong class="message-author"><c:out value="${chatMessage.authorName}" /></strong>
                                    ...
                                    <time class="message-time"><c:out value="${chatMessage.createdLabel}" /></time>
                                </header>
                                <p class="message-text"><c:out value="${chatMessage.message}" /></p>
                                <c:if test="${sessionScope.role eq 'ADMIN' or isOwn}">
                                    <form class="message-delete-form" method="post" action="${pageContext.request.contextPath}/discussions/messages/delete" onsubmit="return confirm('Delete this message?');">
                                        ...
                                        <button type="submit" class="message-delete-btn" title="Delete message">Delete</button>
                                    </form>
                                </c:if>
                            </div>
                            <div class="avatar avatar-chat"><c:out value="${chatMessage.authorName.substring(0,1)}" /></div>
                        </c:when>
                        <c:otherwise>
                            <div class="avatar avatar-chat"><c:out value="${chatMessage.authorName.substring(0,1)}" /></div>
