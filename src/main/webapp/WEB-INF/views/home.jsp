...
<div class="dashboard-shell">
    ...
    <aside class="dashboard-right">
        <section class="side-card"><div class="side-card-heading"><h2>Upcoming Deadlines</h2><a href="${pageContext.request.contextPath}/deadlines">View all</a></div><c:choose><c:when test="${empty deadlines}"><p class="empty-side-text">No upcoming deadlines.</p></c:when><c:otherwise><div class="deadline-list"><c:forEach var="deadline" items="${deadlines}"><div class="deadline-item"><span class="deadline-dot"></span><div><strong><c:out value="${deadline.title}" /></strong><small><c:out value="${deadline.subjectName}" /> · Due <c:out value="${deadline.dueLabel}" /></small></div></div></c:forEach></div></c:otherwise></c:choose></section>
