            ...
            <section class="profile-hero">
                <div class="profile-hero-content">
                    <div class="profile-hero-identity">
                        ...
                        <div class="profile-identity-meta"><span class="profile-role role-${profile.role}"><c:out value="${profile.role}" /></span><p class="presence-status ${activeNow ? 'is-active' : ''}"><span aria-hidden="true"></span><c:out value="${presenceLabel}" /></p></div>
                        </div>
                        </div>
                        ...
                        <form method="post" action="${pageContext.request.contextPath}/profile" class="profile-form" enctype="multipart/form-data" data-profile-photo-form>
                            ...
                            <c:if test="${profile.role ne 'ADMIN'}"><c:choose>
                                <c:when test="${profile.academicInfoLocked}">
                                    ...
                                    <div class="locked-field">
                                        <span>${profile.semester le 6 ? 'Major' : 'Section'}</span>
                                ...
                                <c:otherwise>
                                    <div class="profile-academic-picker" data-academic-group-picker>
                                    <div>
                                        <label for="semester">Semester</label>
                                        <select class="form-select" id="semester" name="semester" data-group-semester>
                                    ...
                                    <div>
                                        <label for="sectionName" data-group-label>${profile.semester le 6 ? 'Major' : 'Section'}</label>
                                    ...
                                    <p class="profile-security-note">Semester and academic group control access to scoped announcements, deadlines, and discussions. Once saved, they will be locked.</p>
                                </c:otherwise>
                            </c:choose></c:if>
                    ...
                    <div class="profile-grid">
                        ...
                        <c:if test="${profile.role ne 'ADMIN'}"><section class="profile-card">
                            ...
                            <c:choose>
                                    ...
                                    <c:choose>
                                        <c:when test="${not empty profile.semester and not empty profile.sectionName}">
                                            <dl class="profile-details">
                                                ...
                                                <div><dt>${profile.semester le 6 ? 'Major' : 'Section'}</dt><dd><c:out value="${profile.sectionName}" /></dd></div>
                                ...
                                <c:otherwise>
                                    <dl class="profile-details">
                                        ...
                                        <div><dt>${profile.semester le 6 ? 'Major' : 'Section'}</dt><dd><c:choose><c:when test="${empty profile.sectionName}">Not assigned</c:when><c:otherwise><c:out value="${profile.sectionName}" /></c:otherwise></c:choose></dd></div>
                                    </dl>
                                </c:otherwise>
                            </c:choose>
                            <c:if test="${not publicProfile}"><c:choose>
                                    ...
                                    <c:choose>
                                        <c:when test="${not empty pendingAcademicRequest}">
                                            <div class="alert alert-info mt-3">
                                                <strong>Academic change request pending:</strong> Requested Semester <c:out value="${pendingAcademicRequest.requestedSemester}" />, ${pendingAcademicRequest.requestedSemester le 6 ? 'Major' : 'Section'} <c:out value="${pendingAcademicRequest.requestedSection}" />.
                                                ...
                                                <form method="post" action="${pageContext.request.contextPath}/profile/academic-change" class="profile-form mt-3">
                                                    ...
                                                    <div class="student-current-academic-card">
                                                        ...
                                                        <div class="student-current-academic-grid">
                                                            ...
                                                            <div class="student-current-academic-item">
                                                                <span class="item-label">Current ${profile.semester le 6 ? 'major' : 'section'}</span>
                                                                <span class="item-value"><c:out value="${profile.sectionName}" /></span>
                                                            </div>
                                                        </div>
                                                    </div>
                                                    <div class="profile-academic-picker" data-academic-group-picker data-group-label-prefix="Requested ">
                                                    <div class="mb-3">
                                                        <label class="form-label" for="reqSemester">Requested semester</label>
                                                        <select class="form-select" id="reqSemester" name="semester" data-group-semester required>
                                                            <c:forEach begin="1" end="10" var="number">
                                                                <option value="${number}" <c:if test="${profile.semester eq number}">selected</c:if>>Semester ${number}</option>
                                                            </c:forEach>
                                                        </select>
                                                    ...
                                                    <div class="mb-3">
                                                        <label class="form-label" for="reqSection" data-group-label>Requested ${profile.semester le 6 ? 'major' : 'section'}</label>
                                                    ...
                                                    <div class="mb-3">
                                                        ...
                                                        <textarea class="form-control" id="reqReason" name="reason" minlength="10" maxlength="1000" rows="3" placeholder="Explain why you need to change your semester or academic group (10-1000 characters)" required></textarea>
                                                    </div>
                                ...
                                <c:otherwise>
                                    <p class="profile-card-note text-primary">Academic information is not set yet. Click "Edit profile" above to choose your semester and academic group.</p>
                                </c:otherwise>
                            </c:choose></c:if>
                        </section></c:if>
