...
<head>
    ...
    <title>Edit Announcement | StudentHub</title>
...
<main class="container py-5">
    <div class="auth-card create-post-card card border-0 shadow-lg mx-auto">
        <div class="card-body p-4 p-md-5">
            <a href="${pageContext.request.contextPath}/announcements" class="text-decoration-none">← Back to announcements</a>
            <h1 class="h2 mt-3">Edit announcement</h1>

            <c:if test="${not empty error}">
                <div class="alert alert-danger"><c:out value="${error}" /></div>
            </c:if>

            <form method="post" action="${pageContext.request.contextPath}/posts/edit" enctype="multipart/form-data" novalidate>
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select" id="categoryId" name="categoryId" required>
                        <c:forEach var="category" items="${categories}">
                            <option value="${category.categoryId}" data-name="${category.categoryName}" <c:if test="${post.categoryId eq category.categoryId}">selected</c:if>><c:out value="${category.categoryName}" /></option>
                        </c:forEach>
                    </select>
                </div>
                ...
                <div class="mb-3">
                    ...
                    <textarea class="form-control" id="content" name="content" rows="6" maxlength="10000" required><c:out value="${post.content}" /></textarea>
                </div>
                ...
                <div class="card p-3 mb-3 bg-body-tertiary border-0 rounded-3" id="deadlineContainer">
                    ...
                    <div class="row g-2">
                        <div class="col-md-7">
                            ...
                            <input class="form-control" type="datetime-local" id="dueDate" name="dueDate" value="${existingDeadline != null ? existingDeadline.inputDueDate : ''}">
                            <div class="form-text" id="deadlineHelp">Required for Assignment, Tutorial, and Exam announcements.</div>
                        </div>
                        <div class="col-md-5">
                            <label class="form-label" for="subjectName">Subject / Course</label>
                            <input class="form-control" id="subjectName" name="subjectName" maxlength="100" value="<c:out value='${existingDeadline != null ? existingDeadline.subjectName : ""}' />" placeholder="e.g. Data Structures">
                        </div>
                    </div>
                </div>

                <div class="mb-4">
                    <label class="form-label" for="attachment">Add Attachment (Image, Document, Video)</label>
                    ...
                    <div class="form-text">Supported: Images, Documents, Videos. Max 50MB.</div>
                </div>
