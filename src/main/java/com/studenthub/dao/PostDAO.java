...
public class PostDAO {
    ...
    public long create(Connection connection, long userId, long categoryId, String title, String content,
                       String visibility)
            throws SQLException {
        String sql = "INSERT INTO posts (user_id, category_id, title, content, visibility) "
                + "VALUES (?, ?, ?, ?, ?)";
    ...
    public int update(Connection connection, long postId, long categoryId, String title,
        ...
        try (PreparedStatement statement = connection.prepareStatement(
                "UPDATE posts SET category_id=?, title=?, content=?, visibility=? WHERE post_id=?")) {
            statement.setLong(1, categoryId); statement.setString(2, title);
            statement.setString(3, content); statement.setString(4, visibility); statement.setLong(5, postId);
