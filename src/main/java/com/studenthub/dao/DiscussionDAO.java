...
public class DiscussionDAO {
    public record AcademicProfile(Integer semester, String sectionName) {}
    ...
    public AcademicProfile findAcademicProfile(long userId) throws SQLException {
        String sql = "SELECT semester, section_name FROM users WHERE user_id = ?";
        try (Connection connection = DBConnection.getConnection();
             PreparedStatement statement = connection.prepareStatement(sql)) {
            statement.setLong(1, userId);
            try (ResultSet results = statement.executeQuery()) {
                if (!results.next()) return new AcademicProfile(null, null);
                int value = results.getInt("semester");
                return new AcademicProfile(results.wasNull() ? null : value, results.getString("section_name"));
            }
        }
    }
    ...
    public List<DiscussionMessage> findRecent(DiscussionTarget target, int limit) throws SQLException {
        StringBuilder sql = new StringBuilder("""
                ...
                WHERE r.room_type = ?
                """);
        if (target.scope() == DiscussionScope.SEMESTER) sql.append(" AND r.semester = ? AND r.section_name IS NULL");
        if (target.scope() == DiscussionScope.SECTION) sql.append(" AND r.semester = ? AND r.section_name = ?");
        if (target.scope() == DiscussionScope.ALL) sql.append(" AND r.semester IS NULL AND r.section_name IS NULL");
        ...
        try (Connection connection = DBConnection.getConnection();
            ...
            if (target.scope() != DiscussionScope.ALL) statement.setInt(index++, target.semester());
            ...
            try (ResultSet results = statement.executeQuery()) {
                while (results.next()) {
                    messages.add(new DiscussionMessage(results.getLong("message_id"),
                            ...
                            results.getString("role"), results.getString("message"),
