...
public class DeadlinesServlet extends HttpServlet{
    ...
    @Override protected void doGet(HttpServletRequest request,HttpServletResponse response)throws ServletException,IOException{
        ...
        try{request.setAttribute("upcomingDeadlines",deadlineDAO.findUpcomingForUser(userId,100));request.setAttribute("pastDeadlines",deadlineDAO.findPastForUser(userId,50));}
        catch(SQLException exception){getServletContext().log("Deadlines load failed: "+exception.getClass().getName());request.setAttribute("upcomingDeadlines",List.of());request.setAttribute("pastDeadlines",List.of());request.setAttribute("error","Deadlines are temporarily unavailable.");}
